  ...
  >
    ...
    >
      ...
      <el-form-item label="Brand logo url" prop="logo">
        <!-- <el-input v-model="dataForm.logo" placeholder="Brand logo url"></el-input> -->
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {
        name: [{ required: true, message: "Brand name不能为空", trigger: "blur" }],
        logo: [
          { required: true, message: "Brand logo url不能为空", trigger: "blur" }
        ],
        descript: [
        ...
        showStatus: [
          {
            required: true,
            message: "Show status[0-不显示；1-显示]不能为空",
            trigger: "blur"
          }
        ],
        firstLetter: [
        ...
        sort: [
          {
            validator: (rule, value, callback) => {
              if (value == "") {
                callback(new Error("Sort字段必须填写"));
              } else if (!Number.isInteger(value) || value<0) {
                callback(new Error("Sort必须是一个大于等于0的整数"));
              } else {
                callback();
              }
            },
