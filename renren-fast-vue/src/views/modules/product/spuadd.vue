    ...
    <el-row>
      <el-col :span="24">
        <el-steps :active="step" finish-status="success">
          ...
          <el-step title="Sales attributes"></el-step>
      ...
      <el-col :span="24" v-show="step==0">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            <el-form-item label="Goods Name" prop="spuName">
            ...
            <el-form-item label="Points" prop="bounds">
              <label>coin</label>
              <el-input-number style="width:130px" placeholder="coin" v-model="spu.bounds.buyBounds" :min="0"
                controls-position="right"></el-input-number>
              <label style="margin-left:15px">Exp</label>
              <el-input-number style="width:130px" placeholder="Exp" v-model="spu.bounds.growBounds" :min="0"
                controls-position="right">
                <template slot="prepend">Exp</template>
              </el-input-number>
            </el-form-item>
            <el-form-item label="Product description(images)" label-width="200px" prop="decript">
              <multi-upload v-model="spu.decript"></multi-upload>
            </el-form-item>

            <el-form-item label="Product Atlas" prop="images" label-width="200px">
              <multi-upload v-model="spu.images"></multi-upload>
            </el-form-item>
            <el-form-item>
              <el-button type="success" @click="collectSpuBaseInfo">Next：Set specifications</el-button>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
              ...
              <el-form ref="form" :model="spu">
                <el-form-item :label="attr.attrName" v-for="(attr,aidx) in group.attrs" :key="attr.attrId">
                  ...
                  <el-select v-model="dataResp.baseAttrs[gidx][aidx].attrValues" :multiple="attr.valueType == 1"
                    filterable allow-create default-first-option placeholder="Select or input">
                    <el-option v-for="(val,vidx) in attr.valueSelect.split(';')" :key="vidx" :label="val" :value="val">
                    </el-option>
                  </el-select>
                  <el-checkbox v-model="dataResp.baseAttrs[gidx][aidx].showDesc" :true-label="1" :false-label="0">Show in summary
                  </el-checkbox>
                </el-form-item>
              </el-form>
            </el-tab-pane>
      ...
      <el-col :span="24" v-show="step==2">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-card class="box-card">
                  ...
                  <el-checkbox-group v-model="dataResp.tempSaleAttrs[aidx].attrValues">
                    ...
                    <div style="margin-left:20px;display:inline">
                      <el-button v-show="!inputVisible[aidx].view" class="button-new-tag" size="mini"
                        @click="showInput(aidx)">+Add</el-button>
            ...
            <el-button type="success" @click="generateSkus">Next: Set SKU information</el-button>
          </el-card>
        </el-card>
      </el-col>
      <el-col :span="24" v-show="step==3">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-table :data="spu.skus" style="width: 100%" :default-expand-all="true">
            ...
            <el-table-column label="Name" prop="skuName">
              <template slot-scope="scope">
            ...
            <el-table-column type="expand" >
              <template slot-scope="scope">
                <el-row>
                  <el-col :span="24">
                    <label style="display:block;float:left">Select Images</label>
                          ...
                          <el-col :span="12">
                            <el-tag v-if="scope.row.images[index].defaultImg == 1">
                              <input type="radio" checked :name="scope.row.skuName"
                                @change="checkDefaultImg(scope.row,index,img)" />Set default
                            </el-tag>
                            <el-tag v-else>
                  ...
                  <el-row>
                    <el-col :span="24">
                      <el-form-item label="Set Discount">
                        ...
                        <el-input-number style="width:160px" v-model="scope.row.discount" :precision="0" :max="100"
                          :min="0" :step="5" controls-position="right"></el-input-number>
                        <label>off</label>
                        <el-checkbox v-model="scope.row.countStatus" :true-label="1" :false-label="0">Can use with other promotion
                        </el-checkbox>
                      </el-form-item>
                    </el-col>
                    <el-col :span="24">
                      <el-form-item label="Set Promotion">
                        <label>$</label>
                        ...
                        <el-input-number style="width:160px" v-model="scope.row.reducePrice" :step="10" :min="0"
                          controls-position="right"></el-input-number>
                        <label>off</label>
                        <el-checkbox v-model="scope.row.priceStatus" :true-label="1" :false-label="0">Can use with other promotion
                        </el-checkbox>
                      </el-form-item>
                    </el-col>

                    <el-col :span="24">
                      <el-form-item label="Membership discount price" v-if="scope.row.memberPrice.length>0">
                        <br />
          ...
          <el-button type="success" @click="submitSkus">Next: Save</el-button>
        </el-card>
      </el-col>
      <el-col :span="24" v-show="step==4">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <h1>Save successfully!</h1>
          <el-button type="primary" @click="addAgian">Add another one</el-button>
        </el-card>
      </el-col>
    </el-row>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      spuBaseInfoRules: {
        ...
        catalogId: [
          { required: true, message: "Please select aCategory", trigger: "blur" }
        ],
        brandId: [
          { required: true, message: "Please select aBrand", trigger: "blur" }
        ],
        decript: [
  ...
  methods: {
    ...
    checkDefaultImg(row, index, img) {
      ...
      row.images[index].defaultImg = 1; //Edit标志位;
      //Edit其他人的标志位
    ...
    submitSkus() {
      ...
      this.$confirm("This operation need some time, continue?", "", {
        ...
        .then(() => {
          this.$http({
            ...
            if (data.code == 0) {
              this.$message({
                type: "success",
                message: "Addproductsuccessfully!"
              });
              this.step = 4;
            } else {
              this.$message({
