  ...
  <div class="mod-schedule">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('sys:schedule:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">Delete</el-button>
        <el-button v-if="isAuth('sys:schedule:pause')" type="danger" @click="pauseHandle()" :disabled="dataListSelections.length <= 0">暂停</el-button>
        <el-button v-if="isAuth('sys:schedule:resume')" type="danger" @click="resumeHandle()" :disabled="dataListSelections.length <= 0">恢复</el-button>
        <el-button v-if="isAuth('sys:schedule:run')" type="danger" @click="runHandle()" :disabled="dataListSelections.length <= 0">立即执行</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`Confirm对[id=${ids.join(',')}]进行[${id ? 'Delete' : 'Delete'}]Operation?`, '', {
      ...
      pauseHandle (id) {
        ...
        this.$confirm(`Confirm对[id=${ids.join(',')}]进行[${id ? '暂停' : '暂停'}]Operation?`, '', {
          confirmButtonText: 'Confirm',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
      ...
      resumeHandle (id) {
        ...
        this.$confirm(`Confirm对[id=${ids.join(',')}]进行[${id ? '恢复' : '恢复'}]Operation?`, '', {
          confirmButtonText: 'Confirm',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
      ...
      runHandle (id) {
        ...
        this.$confirm(`Confirm对[id=${ids.join(',')}]进行[${id ? '立即执行' : '立即执行'}]Operation?`, '', {
          confirmButtonText: 'Confirm',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
